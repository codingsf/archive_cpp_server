...
string BalanceHandle::getRealQueue(const string& dev_id, const string& queue_name)
{
	if(this->mod == 1)
	{
		return queue_name;
...
session::HANDLE_RTN BalanceHandle::cidLogin(const protocol::PacketIn* in, protocol::PacketOut* out)
{
	...
	else
	{
		...
		string queue = getRealQueue(in->getDevId(), "msg_upload");
