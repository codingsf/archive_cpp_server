...
namespace common
{
	...
	void Conn::run()
	{
		if(conn_type & CONN_TYPE_LISTENER)
		{
			...
			if(!pEvConn)
			{
				LOG_DEBUG("service on " << addr << ":" << ntohs(sAddrIn.sin_port) << " fail");
				return;
			}

			LOG_DEBUG("service on " << addr << ":" << ntohs(sAddrIn.sin_port));
		...
		if(conn_type & CONN_TYPE_CONNECTER)
		{
			...
			{
				LOG_DEBUG("connection to " << addr << ":" << ntohs(sAddrIn.sin_port) << " fail");
				bufferevent_free(this->pConnEvBuf);
				return;
			}

			LOG_DEBUG("connection to " << addr << ":" << ntohs(sAddrIn.sin_port));
