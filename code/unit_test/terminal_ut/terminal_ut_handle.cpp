...
session::HANDLE_RTN TerminalUtHandle::conn(protocol::PacketOut* out)
{
		...
		// return cidDataExposure_build(out);
		// return cidDataShock_build(out);
		// return cidDataTilt_build(out);
		// return cidDataBaro_build(out);
	#else
...
session::HANDLE_RTN TerminalUtHandle::cidDataLbs_build(protocol::PacketOut* out)
{
	...
	lbs_1.ld[0].mnc = 1;
	lbs_1.ld[0].mcc = 460;
	lbs_1.ld[0].lac = 39185;
	lbs_1.ld[0].cell_id = 42376;
	lbs_1.ld[0].bsic = 35;
	lbs_1.ld[0].rssi = 7;
...
session::HANDLE_RTN TerminalUtHandle::cidDevband(const protocol::PacketIn* in, protocol::PacketOut* out)
{
	...
	for(TLVGroupIter iter = pTlvGrp->begin(); iter != pTlvGrp->end(); ++iter)
	{
		...
		if(iter->type == TID_ORDER_ID)
		{
			...
			memcpy(&buf, iter->value, iter->len);

			LOG_DEBUG("cidDevband - TID_ORDER_ID - " << buf);
		}
...
session::HANDLE_RTN TerminalUtHandle::cidDataTh2(const protocol::PacketIn* in, protocol::PacketOut* out)
{
	...
	for(TLVGroupIter iter = pTlvGrp->begin(); iter != pTlvGrp->end(); ++iter)
	{
		if(iter->type == TID_RTN)
		{
			LOG_DEBUG("CID_DATA_TH2 - TID_RTN - " << (int)*iter->value);
		}
	}
...
session::HANDLE_RTN TerminalUtHandle::cidEFence(const protocol::PacketIn* in, protocol::PacketOut* out)
{
	TLVGroupPtr pTlvGrp = in->getTLVGroup();
	for(TLVGroupIter iter = pTlvGrp->begin(); iter != pTlvGrp->end(); ++iter)
	{
		if(iter->type == TID_RTN)
		{
			LOG_DEBUG("CID_E_FENCE - TID_RTN - " << (int)*iter->value);
		}
	}
...
session::HANDLE_RTN TerminalUtHandle::cidDataExposure_build(protocol::PacketOut* out)
{
	out->setDevId("201506040000");
	out->setCID(CID_DATA_EXPOSURE);

	TLV_EXPOSURE exposure_1;

	exposure_1.dt = buildDateTime2k();
	exposure_1.dt_gps = buildDateTime2k();

	exposure_1.alarm = 0;
	exposure_1.exposure = 288;

	out->add(TID_EXPOSURE, sizeof(TLV_EXPOSURE), (char*)&exposure_1);

	sleep(1);

	TLV_EXPOSURE exposure_2;

	exposure_2.dt = buildDateTime2k();
	exposure_2.dt_gps = buildDateTime2k();

	exposure_2.alarm = 1;
	exposure_2.exposure = 228;

	out->add(TID_EXPOSURE, sizeof(TLV_EXPOSURE), (char*)&exposure_2);
...
session::HANDLE_RTN TerminalUtHandle::cidDataExposure(const protocol::PacketIn* in, protocol::PacketOut* out)
{
	TLVGroupPtr pTlvGrp = in->getTLVGroup();
	for(TLVGroupIter iter = pTlvGrp->begin(); iter != pTlvGrp->end(); ++iter)
	{
		if(iter->type == TID_RTN)
		{
			LOG_DEBUG("CID_DATA_EXPOSURE - TID_RTN - " << (int)*iter->value);
		}
	}
...
session::HANDLE_RTN TerminalUtHandle::cidDataShock_build(protocol::PacketOut* out)
{
	out->setDevId("201506040000");
	out->setCID(CID_DATA_SHOCK);

	TLV_SHOCK shock_1;

	shock_1.dt = buildDateTime2k();
	shock_1.dt_gps = buildDateTime2k();
	...
	out->add(TID_SHOCK, sizeof(TLV_SHOCK), (char*)&shock_1);

	sleep(1);

	TLV_SHOCK shock_2;

	shock_2.dt = buildDateTime2k();
	shock_2.dt_gps = buildDateTime2k();
	...
	out->add(TID_SHOCK, sizeof(TLV_SHOCK), (char*)&shock_2);
...
session::HANDLE_RTN TerminalUtHandle::cidDataShock(const protocol::PacketIn* in, protocol::PacketOut* out)
{
	TLVGroupPtr pTlvGrp = in->getTLVGroup();
	for(TLVGroupIter iter = pTlvGrp->begin(); iter != pTlvGrp->end(); ++iter)
	{
		if(iter->type == TID_RTN)
		{
			LOG_DEBUG("CID_DATA_SHOCK - TID_RTN - " << (int)*iter->value);
		}
	}
...
session::HANDLE_RTN TerminalUtHandle::cidDataTilt_build(protocol::PacketOut* out)
{
	out->setDevId("201506040000");
	out->setCID(CID_DATA_TILT);

	TLV_TILT tilt_1;

	tilt_1.dt = buildDateTime2k();
	tilt_1.dt_gps = buildDateTime2k();
	...
	out->add(TID_TILT, sizeof(TLV_TILT), (char*)&tilt_1);

	sleep(1);

	TLV_TILT tilt_2;

	tilt_2.dt = buildDateTime2k();
	tilt_2.dt_gps = buildDateTime2k();
	...
	out->add(TID_TILT, sizeof(TLV_TILT), (char*)&tilt_2);
...
session::HANDLE_RTN TerminalUtHandle::cidDataTilt(const protocol::PacketIn* in, protocol::PacketOut* out)
{
	TLVGroupPtr pTlvGrp = in->getTLVGroup();
	for(TLVGroupIter iter = pTlvGrp->begin(); iter != pTlvGrp->end(); ++iter)
	{
		if(iter->type == TID_RTN)
		{
			LOG_DEBUG("CID_DATA_TILT - TID_RTN - " << (int)*iter->value);
		}
	}
...
session::HANDLE_RTN TerminalUtHandle::cidDataBaro_build(protocol::PacketOut* out)
{
	out->setDevId("201506040000");
	out->setCID(CID_DATA_BARO);

	TLV_BARO baro_1;

	baro_1.dt = buildDateTime2k();
	baro_1.dt_gps = buildDateTime2k();

	baro_1.alarm = 0;
	baro_1.baro = 288;

	out->add(TID_BARO, sizeof(TLV_BARO), (char*)&baro_1);

	sleep(1);

	TLV_BARO baro_2;

	baro_2.dt = buildDateTime2k();
	baro_2.dt_gps = buildDateTime2k();

	baro_2.alarm = 1;
	baro_2.baro = 228;

	out->add(TID_BARO, sizeof(TLV_BARO), (char*)&baro_2);
...
session::HANDLE_RTN TerminalUtHandle::cidDataBaro(const protocol::PacketIn* in, protocol::PacketOut* out)
{
	TLVGroupPtr pTlvGrp = in->getTLVGroup();
	for(TLVGroupIter iter = pTlvGrp->begin(); iter != pTlvGrp->end(); ++iter)
	{
		if(iter->type == TID_RTN)
		{
			LOG_DEBUG("CID_DATA_BARO - TID_RTN - " << (int)*iter->value);
		}
	}
