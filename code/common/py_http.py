...
#zengpw mark - 任何异常产生或抛出都会导致C++端python调用环境破坏，从而导致后续调用异常
def requestAmap(req):
	...
	try:
		...
		#status == 0代表出错
		j_status = content["status"]
		# print("py_http.py : " + j_status)
		if j_status == "0":
		...
		T = (L[0], L[1])
		return T
	#网络不正常，则抛出错误给 C++再次处理（暂时没法处理，返回全0）
	except urllib.error.HTTPError as e:
		# print(e.code)
		# print(e.read().decode("utf8"))
		return ("000000000000", "000000000000")
	#如果出现不可控的异常，则让 C++处理（暂时没法处理，返回全0）
	except:
...
# if __name__ == '__main__':
	...
	# requestAmap("http://apilocate.amap.com/position?accesstype=0&imei=866104021101384&cdma=0&bts=460,0,6317,12609,-72&nearbts=460,0,6317,12609,-72|460,0,6317,12609,-72&output=json&key=fe0d26ade7f0dc72e2d2efa4d10fc259")
	...
	# requestAmapUseAddr("http://restapi.amap.com/v3/geocode/regeo?location=0.000,0.000&extensions=all&output=json&key=44e606a9c5a628e9a8bd42226249f84c")
