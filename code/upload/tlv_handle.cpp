...
TLV_HANDLE_RTN TlvHandle::tlv_firm_handle_ver11(TLV_DEV_FIRM_V11& firm, TLV_HANDLE_DEV_FIRM_V11& firm_handle)
{
	LOG_DEBUG("====== tlv_firm_handle_ver11 processing ======");

	//-------------- 打印入参
	LOG_DEBUG("---------- TLV_DEV_FIRM_V11 content : ");
	LOG_DEBUG("version = " << firm.version << \
			", imei = " << firm.imei << \
			", iccid = " << firm.iccid << \
			", mac = " << firm.mac);
	...
	//-------------- 打印出参
	LOG_DEBUG("---------- TLV_HANDLE_DEV_FIRM_V11 content : ");
	LOG_DEBUG("dev_id = " << firm_handle.dev_id << \
			", version = " << firm_handle.version << \
			", imei = " << firm_handle.imei << \
			", iccid = " << firm_handle.iccid << \
...
TLV_HANDLE_RTN TlvHandle::tlv_lbs_handle(TLV_LBS& lbs, TLV_HANDLE_LBS& lbs_handle)
{
	...
	else
	{
		...
		//经纬度长度判定
		size_t lat_idx = lbs_handle.latitude.find_first_of('.');
		size_t long_idx = lbs_handle.longitude.find_first_of('.');
		...
		//补到12位长度
		if(lat_idx == string::npos || long_idx == string::npos){
			lbs_handle.latitude = "000000000000";
			lbs_handle.longitude = "000000000000";
		}
		else
		{
			if(lat_idx == 1){
				memset(buf, 0, sizeof(buf));
				sprintf(buf, "00%s", lbs_handle.latitude.c_str());
				ss << buf;
			...
			if(long_idx == 1){
				memset(buf, 0, sizeof(buf));
				sprintf(buf, "00%s", lbs_handle.longitude.c_str());
				ss << buf;
				lbs_handle.longitude = ss.str() + "000000000000";
			...
			if(long_idx == 2){
				memset(buf, 0, sizeof(buf));
				sprintf(buf, "0%s", lbs_handle.longitude.c_str());
				ss << buf;
				lbs_handle.longitude = ss.str() + "000000000000";
				lbs_handle.longitude = lbs_handle.longitude.substr(0, 12);
				ss.str("");
...
TLV_HANDLE_RTN TlvHandle::tlv_th_handle_ver12(TLV_TH_V12& th, TLV_HANDLE_TH& th_handle)
{
	LOG_DEBUG("====== tlv_th_handle_ver12 processing ======");

	//-------------- 打印入参
	LOG_DEBUG("---------- TLV_TH_V12 content : ");
	printDateTime2k(th.dt);
	printDateTime2k(th.dt_gps);
	LOG_DEBUG("alarm = " << th.alarm << \
			", t0 = " << th.t0 << \
			", h0 = " << th.h0);
	...
	if(th.alarm[0] != '\0')
	{
		th_handle.alarm.assign((char*)th.alarm, sizeof(th.alarm));
		reverse(th_handle.alarm.begin(), th_handle.alarm.end());
	}
	else
	{
		th_handle.alarm = "00000000";
	}
...
TLV_HANDLE_RTN TlvHandle::tlv_dev_type_handle(TLV_DEV_TYPE& tdt, TLV_HANDLE_DEV_TYPE& dev_type_handle)
{
	LOG_DEBUG("====== tlv_dev_type_handle processing ======");

	//-------------- 打印入参
	LOG_DEBUG("---------- TLV_DEV_TYPE content : ");
	LOG_DEBUG("type = " << tdt.type);
	LOG_DEBUG("---------- end.");

	stringstream ss;

	ss << tdt.type;
	dev_type_handle.type = ss.str();
	ss.str("");
	ss.clear();

	//-------------- 打印出参
	LOG_DEBUG("---------- TLV_HANDLE_DEV_TYPE content : ");
	LOG_DEBUG("type = " << dev_type_handle.type);
	LOG_DEBUG("---------- end.");
...
TLV_HANDLE_RTN TlvHandle::tlv_manage_lbs_handle(TLV_MANAGE_LBS& m_lbs, TLV_HANDLE_MANAGE_LBS& m_lbs_handle)
{
	...
	{
		hi.gps2addrAmap(m_lbs_handle.latitude, m_lbs_handle.longitude, m_lbs_handle.addr);

		//经纬度长度判定
		size_t lat_idx = m_lbs_handle.latitude.find_first_of('.');
		size_t long_idx = m_lbs_handle.longitude.find_first_of('.');
		...
		else
		{
			if(lat_idx == 1){
				memset(buf, 0, sizeof(buf));
				sprintf(buf, "00%s", m_lbs_handle.latitude.c_str());
				ss << buf;
			...
			if(long_idx == 1){
				memset(buf, 0, sizeof(buf));
				sprintf(buf, "00%s", m_lbs_handle.longitude.c_str());
				ss << buf;
				m_lbs_handle.longitude = ss.str() + "000000000000";
			...
			if(long_idx == 2){
				memset(buf, 0, sizeof(buf));
				sprintf(buf, "0%s", m_lbs_handle.longitude.c_str());
				ss << buf;
				m_lbs_handle.longitude = ss.str() + "000000000000";
				m_lbs_handle.longitude = m_lbs_handle.longitude.substr(0, 12);
				ss.str("");
...
TLV_HANDLE_RTN TlvHandle::tlv_th2_handle(TLV_TH2& th, TLV_HANDLE_TH2& th_handle)
{
	...
	//-------------- 生成额外数据
	if(th.t0 > 900 || th.t0 < -900 || th.t1 > 900 || th.t1 < -900)
		th_handle.overflow = "1";
	else
		th_handle.overflow = "0";
...
TLV_HANDLE_RTN TlvHandle::tlv_th2_handle_ver12(TLV_TH2_V12& th, TLV_HANDLE_TH2& th_handle)
{
	LOG_DEBUG("====== tlv_th2_handle_ver12 processing ======");

	//-------------- 打印入参
	LOG_DEBUG("---------- TLV_TH2_V12 content : ");
	printDateTime2k(th.dt);
	printDateTime2k(th.dt_gps);
	LOG_DEBUG("alarm = " << th.alarm << \
			", t0 = " << th.t0 << \
...
TLV_HANDLE_RTN TlvHandle::tlv_band_time_handle(TLV_TIME_6& band_time, TLV_HANDLE_DEVBAND_TIME& band_time_handle)
{
	LOG_DEBUG("====== tlv_band_time_handle processing ======");

	//-------------- 打印入参
	LOG_DEBUG("---------- TLV_TIME_6 content : ");
	printDateTime2k(band_time.dt);
	LOG_DEBUG("---------- end.");

	band_time_handle.datetime = DateTime2String2k(band_time.dt);

	//-------------- 打印出参
	LOG_DEBUG("---------- TLV_HANDLE_DEVBAND_TIME content : ");
	LOG_DEBUG("dev_id = " << band_time_handle.dev_id);
	LOG_DEBUG("datetime = " << band_time_handle.datetime);
	LOG_DEBUG("---------- end.");

	return THR_OK;
}

TLV_HANDLE_RTN TlvHandle::tlv_jzt_dev_handle(TLV_JZT_DEV& jzt_dev, TLV_HANDLE_JZT_DEV& jzt_dev_handle)
{
	LOG_DEBUG("====== tlv_jzt_dev_handle processing ======");

	//-------------- 打印入参
	LOG_DEBUG("---------- TLV_JZT_DEV content : ");
	printDateTime2k(jzt_dev.dt);
	LOG_DEBUG("voltage = " << jzt_dev.voltage);
	LOG_DEBUG("---------- end.");
	...
	ss << jzt_dev.voltage;
	jzt_dev_handle.voltage = ss.str();
	ss.str("");
	ss.clear();

	jzt_dev_handle.datetime = DateTime2String2k(jzt_dev.dt);

	//-------------- 打印出参
	LOG_DEBUG("---------- TLV_HANDLE_JZT_DEV content : ");
	LOG_DEBUG("dev_id = " << jzt_dev_handle.dev_id << \
			", voltage = " << jzt_dev_handle.voltage << \
			", datetime = " << jzt_dev_handle.datetime);
	LOG_DEBUG("---------- end.");

	return THR_OK;
}

TLV_HANDLE_RTN TlvHandle::tlv_jzt_lbs_handle(TLV_JZT_LBS& lbs, TLV_HANDLE_JZT_LBS& lbs_handle)
{
	LOG_DEBUG("====== tlv_jzt_lbs_handle processing ======");

	//-------------- 打印入参
	LOG_DEBUG("---------- TLV_JZT_LBS content : ");
	printDateTime2k(lbs.dt);
	for(unsigned int i = 0; i < sizeof(lbs.ld)/sizeof(LbsData); i++)
	{
		LOG_DEBUG("cell = " << (int)lbs.ld[i].cell << \
	...
	else
	{
		...
		//九州通需要带小数点版本
		lbs_handle.latitude = lbs_handle.latitude.substr(0, 3) + string(".") + lbs_handle.latitude.substr(3, 12);
		lbs_handle.longitude = lbs_handle.longitude.substr(0, 3) + string(".") + lbs_handle.longitude.substr(3, 12);
	}
...
TLV_HANDLE_RTN TlvHandle::tlv_jzt_th2_handle(TLV_JZT_TH2& th, TLV_HANDLE_JZT_TH2& th_handle)
{
	LOG_DEBUG("====== tlv_jzt_th2_handle processing ======");

	//-------------- 打印入参
	LOG_DEBUG("---------- TLV_JZT_TH2 content : ");
	printDateTime2k(th.dt);
	printDateTime2k(th.dt_gps);
	LOG_DEBUG("alarm = " << (int)th.alarm << \
			", t0 = " << th.t0 << \
	...
	//-------------- 打印出参
	LOG_DEBUG("---------- TLV_HANDLE_JZT_TH2 content : ");
	LOG_DEBUG("dev_id = " << th_handle.dev_id << \
			", datetime = " << th_handle.datetime);
	LOG_DEBUG("datetime_gps = " << th_handle.datetime_gps);
	LOG_DEBUG("alarm = " << th_handle.alarm << \
