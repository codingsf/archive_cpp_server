...
namespace common
{
	...
	int Redis::redis_lpush(const char* key, const char* value, const int length)
	{
		...
		{
			LOG_DEBUG("redis LPUSH " << key << " error");
			freeReplyObject(r);
			return -1;
		}
		...
		{
			LOG_DEBUG("redis LPUSH return ok, list name = " << key << ", list size = " << r->integer);
			freeReplyObject(r);
		}
	...
	int Redis::redis_rpush(const char* key, const char* value, const int length)
	{
		// LOG_DEBUG("redis key : " << key);
		// LOG_DEBUG("redis value : " << common::printByHex(value, length));

		redisReply* r = (redisReply*)redisCommand(c, "RPUSH %s %b", key, value, (size_t)length);

		if(r == NULL || r->type == REDIS_REPLY_ERROR || (r->type == REDIS_REPLY_STATUS && r->str != REDIS_OK))
		{
			LOG_DEBUG("redis RPUSH " << key << " error");
			freeReplyObject(r);
			return -1;
		}
		...
		if(r->type == REDIS_REPLY_INTEGER)
		{
			LOG_DEBUG("redis RPUSH return ok, list name = " << key << ", list size = " << r->integer);
			freeReplyObject(r);
		}
	...
	int Redis::redis_lrem(const char* key, const char* value, const int length)
	{
		...
		if(r == NULL || r->type == REDIS_REPLY_ERROR || (r->type == REDIS_REPLY_STATUS && r->str != REDIS_OK))
		{
			LOG_DEBUG("redis LREM " << key << " error");
			freeReplyObject(r);
			return -1;
		}
		...
		{
			i = r->integer;
			LOG_DEBUG("redis LREM return ok, list name = " << key << ", number of delete value is " << r->integer);
			freeReplyObject(r);
		}
	...
	int Redis::redis_rpoplpush(const char* key_src, const char* key_dest, char* value, int* len)
	{
		...
		if(r == NULL || r->type == REDIS_REPLY_ERROR || (r->type == REDIS_REPLY_STATUS && r->str != REDIS_OK))
		{
			LOG_DEBUG("redis RPOPLPUSH " << key_src << " error");
			freeReplyObject(r);
			return -1;
		}
		...
		{
			LOG_DEBUG("redis RPOPLPUSH return ok, list name = " << key_src << ", value = " << common::printByHex(r->str, r->len));
	...
	int Redis::redis_move(const char* key, const int db)
	{
		...
		LOG_DEBUG("redis MOVE return error");
		return -1;
	}
	...
	int Redis::redis_keys(const string& vkey, vector<string>& value)
	{
		...
		if(r == NULL)
		{
			LOG_DEBUG("redis KEYS error , vkey not exist : " << vkey.c_str());
			freeReplyObject(r);
			return -1;
		}
		...
		{
			LOG_DEBUG("redis KEYS return , no data found : " << vkey.c_str());
			freeReplyObject(r);
			return 0;
		}
		...
		LOG_DEBUG("redis KEYS return ok , search vkeys : " << vkey.c_str() << " , result size : " << value.size());
		freeReplyObject(r);
	...
	int Redis::redis_exists(const char* key)
	{
		...
		{
			i = r->integer;
			LOG_DEBUG("redis EXISTS " << key <<" return " << i);
			freeReplyObject(r);
		}
	...
	int Redis::redis_del(const char* key)
	{
		LOG_DEBUG("redis delete key : " << key);

		redisReply* r = (redisReply*)redisCommand(c, "DEL %s", key);
		...
		{
			i = r->integer;
			LOG_DEBUG("redis DEL " << key << " return " << i);
			freeReplyObject(r);
		}
